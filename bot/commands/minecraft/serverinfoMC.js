  ...
  async execute(interaction) {
    ...
    if (data) {
      ...
      await mcSchema.create({
        ...
        UserID: interaction.user.id,
        Guild: interaction.guild.id,
      }); //handling the first time this command is used by a new user
    }
    function initServerData(serverIp) {
      //getting the server info and sending it
      fetch(`https://mcapi.us/server/status?ip=${serverIp}`) //fetch it from the api
        .then((response) => response.json()) //convert the response to json
        .then((data) => {
          if (data.server.name === null) {
            //server name does not exist, reply with the specified message saying that it either doesnt exist or is offline
            interaction.reply(
              "This server is either offline or you provided an invalid adress."
            );
          } else {
